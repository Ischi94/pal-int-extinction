...
fourteen_trends_mammals$trend1[is.na(fourteen_trends_mammals$trend1)] <- 0
fourteen_trends_mammals$trend2[is.na(fourteen_trends_mammals$trend2)] <- 0
fourteen_trends_mammals$trend3[is.na(fourteen_trends_mammals$trend3)] <- 0
fourteen_trends_mammals$trend4[is.na(fourteen_trends_mammals$trend4)] <- 0

# Split short term temperature change into warming and cooling:
fourteen_trends_mammals$cooling<-ifelse(fourteen_trends_mammals$change.prev<0, fourteen_trends_mammals$change.prev, NA)
fourteen_trends_mammals$warming<-ifelse(fourteen_trends_mammals$change.prev>0, fourteen_trends_mammals$change.prev, NA)

## Warming
# Iterate through each long term temperature change
vars = names(dplyr::select(fourteen_trends_mammals, trend2:trend4, trend.st1:trend.st10)) 
modelsw4 = lapply(setNames(vars, vars), function(var) {
  form = paste("extinct~warming:", var, "+(1|Genus)")
  glmer(form, data=fourteen_trends_mammals, family="binomial")
})

# Make data frame for model output
warm_fourteen_trends <- data.frame(model=names(dplyr::select(fourteen_trends_mammals, 
                                              trend2:trend4, trend.st1:trend.st10)), intercept=NA, 
...
# Make data frame for model output
cool_fourteen_trends <-data.frame(model=names(dplyr::select(fourteen_trends_mammals, trend2:trend4, trend.st1:trend.st10)), intercept=NA, 
...
# Save output as csv
#write.table(warm_fourteen_trends, "modell_summary_Mammals_w.csv")
#write.table(cool_fourteen_trends, "modell_summary_Mammals_c.csv")
