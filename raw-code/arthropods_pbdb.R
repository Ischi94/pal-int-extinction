...
###----
arthropods_pbdb <- read.csv("pbdb_arthropods.csv")


# clean it a bit
arthropods_pbdb <- subset(arthropods_pbdb, arthropods_pbdb$accepted_rank != "subgenus")

toOmit <- omit(arthropods_pbdb, bin="stg", tax="genus", om="ref", ref="reference_no")
arthropods_pbdb <- arthropods_pbdb[!toOmit,]

# subset the genera with brackets and assign them to the overarching genus
test <- gsub("\\s*\\([^\\)]+\\)","",as.character(arthropods_pbdb$genus))
arthropods_pbdb$genus <- test

# using basic bin lengths to change it to range data
arthropods <- fadlad(arthropods_pbdb, tax ="genus", age =c("max_ma", "min_ma"))

# transform values back to positive
# arthropods$FAD <- -arthropods$FAD
# arthropods$LAD <- -arthropods$LAD

arthropods$genus <- rownames(arthropods)


# merge them together to get taxonomy
arthropods <- merge(arthropods, arthropods_pbdb, by  = "genus")
...
arthropods <- arthropods[!duplicated(arthropods),]

# remove the NA's
arthropods <- subset(arthropods, arthropods$class != "NO_CLASS_SPECIFIED" &
                       arthropods$order != "NO_ORDER_SPECIFIED" &
                       arthropods$family !=  "NO_FAMILY_SPECIFIED")

arthropods <- na.omit(arthropods) # no NA's anymore, so good to go

#arthropods$dur <- arthropods$FAD - arthropods$LAD
#mean(arthropods$dur) #26.49132
#median(arthropods$dur) #15.6
...
# change colnames
colnames(arthropods) <- c("Phylum", "Class", "Order", "Family", "Genus", "FAD.age", "LAD.age")
...
arthropods$FAD_bin <- NA
arthropods$LAD_bin <- NA

for (i in 1:length(arthropods$Genus)) {
  for (j in 1:length(gradstein$stg)) {
    ifelse(gradstein$bottom[j] >= arthropods$FAD.age[i] & gradstein$top[j] < arthropods$FAD.age[i],
           arthropods$FAD_bin[i] <- gradstein$stg[j], NA)
    ifelse(gradstein$bottom[j] > arthropods$LAD.age[i] & gradstein$top[j] <= arthropods$LAD.age[i],
           arthropods$LAD_bin[i] <- gradstein$stg[j], NA)
  }}

#remove singletons as they add noise 
#foram_sing <- subset(forams_nms, forams_nms$FAD_bin == forams_nms$LAD_bin) # 199 singletons
arthropods <- subset(arthropods, arthropods$FAD_bin != arthropods$LAD_bin)

# when forams range to the recent (bin 95), shift this bin to 94 to remove pull of the recent and 
# to enable temp-calculation
arthropods$LAD_bin[arthropods$LAD_bin==95] <- 94
arthropods <- subset(arthropods, arthropods$FAD_bin != arthropods$LAD_bin)
...
rm(dum1, dum2, i, j, arthropods_pbdb, test, toOmit)
...
dumbo <- arthropods
arthropods_safe <- arthropods[,c(5,8,9)]

# add bins with 0 and fill in with 1 for extinction and origination
namevector <- as.character(c(1:94))
arthropods_safe[ , namevector] <- NA
arthropods_safe <- arthropods_safe[, c(4:length(arthropods_safe[0,]))]

for (i in 1:length(arthropods_safe[,1])) {
  arthropods_safe[i,dumbo[i,8]] <- 0 
  arthropods_safe[i,dumbo[i,9]] <- 1
  ifelse(dumbo[i,8]!= dumbo[i,9]-1 & dumbo[i,8]!= dumbo[i,9],
         arthropods_safe[i,(dumbo[i,8]+1):(dumbo[i,9]-1)] <- 0, NA)
  ifelse(dumbo[i,9] == 94, arthropods_safe[i,dumbo[i,9]] <- 0, 
         arthropods_safe[i,dumbo[i,9]] <- 1)
}


# bind it again 
rownames(arthropods_safe) <-  make.names(dumbo[,5], unique=TRUE)


# Reorganise ranges through time data so we can bind it to temperature data
arthropods_safe %<>%  as_tibble() %>%
  mutate(Genus = rownames(arthropods_safe)) %>%
  group_by(Genus) %>%
...
# Now bind the two
temp_arthropods <- full_join(arthropods_safe, isotemp2)

#add taxonomical levels
dumbo <- dumbo[, 2:5]

# Now bind the two
temp_arthropods <- full_join(temp_arthropods, dumbo)

temp_arthropods <- temp_arthropods %>%
...
temp_arthropods$bins <- as.numeric(temp_arthropods$bins)

ori_bin <- temp_arthropods %>%
...
# Now bind the two
test2 <- full_join(temp_arthropods, ori_bin)
...
art_fam <- unique(arthropods$Family)
fam_dat <- data.frame(matrix(ncol =2 , nrow = length(art_fam)))
colnames(fam_dat) <- c("age.ori", "bin.ori")
rownames(fam_dat) <-  c(as.character(art_fam))


for (i in 1:length(fam_dat$age.ori)) {
  fam_dat[i, 2] <- min(arthropods$FAD_bin[arthropods$Family == art_fam[i]])
...
art_ord <- unique(arthropods$Order)
ord_dat <- data.frame(matrix(ncol =2 , nrow = length(art_ord)))
colnames(ord_dat) <- c("age.ori", "bin.ori")
rownames(ord_dat) <-  c(as.character(art_ord))


for (i in 1:length(ord_dat$age.ori)) {
  ord_dat[i, 2] <- min(arthropods$FAD_bin[arthropods$Order == art_ord[i]])
...
art_cla <- unique(arthropods$Class)
cla_dat <- data.frame(matrix(ncol =2 , nrow = length(art_cla)))
colnames(cla_dat) <- c("age.ori", "bin.ori")
rownames(cla_dat) <-  c(as.character(art_cla))


for (i in 1:length(cla_dat$age.ori)) {
  cla_dat[i, 2] <- min(arthropods$FAD_bin[arthropods$Class == art_cla[i]])
...
#remov ori age and bin
four_trends_arthropods <- test5[,-c(14:15)]


# Set bins back to numeric
four_trends_arthropods %<>% transform(bins = as.numeric(as.character(bins)))

# Cleanup
rm(dumbo, art_cla, arthropods, lin1, 
   ord_dat, ori_bin,  test2, test3, test4, test5, age1, bin1, 
   i, art_fam, art_ord, namevector, temp1, temp_arthropods, arthropods_safe, fam_dat, cla_dat)
...
# Set bins back to numeric
four_trends_arthropods %<>% transform(bins = as.factor(as.character(bins)))

# Now bind the two
fourteen_trends_arthropods <- full_join(four_trends_arthropods, isotemp2)


# Set bins back to numeric
fourteen_trends_arthropods %<>% transform(bins = as.numeric(as.character(bins)))

# remove redundant colums
fourteen_trends_arthropods <- fourteen_trends_arthropods[-23487,]
