...
###----
gastropods_pbdb <- read.csv("pbdb_gastropoda.csv")

table(gastropods_pbdb$accepted_rank)

# clean it a bit
gastropods_pbdb <- subset(gastropods_pbdb, gastropods_pbdb$accepted_rank != "subgenus")

toOmit <- omit(gastropods_pbdb, bin="stg", tax="genus", om="ref", ref="reference_no")
gastropods_pbdb <- gastropods_pbdb[!toOmit,]

# subset the genera with brackets and assign them to the overarching genus
test <- gsub("\\s*\\([^\\)]+\\)","",as.character(gastropods_pbdb$genus))
gastropods_pbdb$genus <- test
# using basic bin lengths to change it to range data
gastropods <- fadlad(gastropods_pbdb, tax="genus", age =c("max_ma", "min_ma"))

# transform values back to positive
# gastropods$FAD <- -gastropods$FAD
# gastropods$LAD <- -gastropods$LAD

gastropods$genus <- rownames(gastropods)


# merge them together to get taxonomy
gastropods <- merge(gastropods,gastropods_pbdb, by  = "genus")
...
gastropods <- gastropods[!duplicated(gastropods),]

# remove the NA's
gastropods <- subset(gastropods, gastropods$class != "NO_CLASS_SPECIFIED" &
                       gastropods$order != "NO_ORDER_SPECIFIED" &
                       gastropods$family !=  "NO_FAMILY_SPECIFIED")

gastropods <- na.omit(gastropods) # no NA's anymore, so good to go

#gastropods$dur <- gastropods$FAD - gastropods$LAD
#mean(gastropods$dur) #46.65374
#median(gastropods$dur) #28.8
...
# change colnames
colnames(gastropods) <- c("Phylum", "Class", "Order", "Family", "Genus", "FAD.age", "LAD.age")
...
gastropods$FAD_bin <- NA
gastropods$LAD_bin <- NA

for (i in 1:length(gastropods$Genus)) {
  for (j in 1:length(gradstein$stg)) {
    ifelse(gradstein$bottom[j] >= gastropods$FAD.age[i] & gradstein$top[j] < gastropods$FAD.age[i],
           gastropods$FAD_bin[i] <- gradstein$stg[j], NA)
    ifelse(gradstein$bottom[j] > gastropods$LAD.age[i] & gradstein$top[j] <= gastropods$LAD.age[i],
           gastropods$LAD_bin[i] <- gradstein$stg[j], NA)
  }}

#remove singletons as they add noise 
#foram_sing <- subset(forams_nms, forams_nms$FAD_bin == forams_nms$LAD_bin) # 199 singletons
gastropods <- subset(gastropods, gastropods$FAD_bin != gastropods$LAD_bin)

# when forams range to the recent (bin 95), shift this bin to 94 to remove pull of the recent and 
# to enable temp-calculation
gastropods$LAD_bin[gastropods$LAD_bin==95] <- 94
gastropods <- subset(gastropods, gastropods$FAD_bin != gastropods$LAD_bin)

# subset to the same bins as we currently have for isotemp (>= 14)
gastropods <- subset(gastropods, gastropods$FAD_bin >=14)

#write.table(gastropods, file = "C:/Users/gmath/Documents/4.Semester/Masterarbeit/clean_data/gastropoda.csv")
...
rm(dum1, dum2, i, j, gastropods_pbdb, test, toOmit)
...
dumbo <- gastropods
gastropods_safe <- gastropods[,c(5,8,9)]

# add bins with 0 and fill in with 1 for extinction and origination
namevector <- as.character(c(1:94))
gastropods_safe[ , namevector] <- NA
gastropods_safe <- gastropods_safe[, c(4:length(gastropods_safe[0,]))]

for (i in 1:length(gastropods_safe[,1])) {
  gastropods_safe[i,dumbo[i,8]] <- 0 
  gastropods_safe[i,dumbo[i,9]] <- 1
  ifelse(dumbo[i,8]!= dumbo[i,9]-1 & dumbo[i,8]!= dumbo[i,9],
         gastropods_safe[i,(dumbo[i,8]+1):(dumbo[i,9]-1)] <- 0, NA)
  ifelse(dumbo[i,9] == 94, gastropods_safe[i,dumbo[i,9]] <- 0, 
         gastropods_safe[i,dumbo[i,9]] <- 1)
}


# bind it again 
rownames(gastropods_safe) <-  make.names(dumbo[,5], unique=TRUE)


# Reorganise ranges through time data so we can bind it to temperature data
gastropods_safe %<>%  as_tibble() %>%
  mutate(Genus = rownames(gastropods_safe)) %>%
  group_by(Genus) %>%
...
# Now bind the two
temp_gastropods <- full_join(gastropods_safe, isotemp2)

#add taxonomical levels
dumbo <- dumbo[, 2:5]

# Now bind the two
temp_gastropods <- full_join(temp_gastropods, dumbo)

temp_gastropods <- temp_gastropods %>%
...
temp_gastropods$bins <- as.numeric(temp_gastropods$bins)

ori_bin <- temp_gastropods %>%
...
# Now bind the two
test2 <- full_join(temp_gastropods, ori_bin)
...
gas_fam <- unique(gastropods$Family)
fam_dat <- data.frame(matrix(ncol =2 , nrow = length(gas_fam)))
colnames(fam_dat) <- c("age.ori", "bin.ori")
rownames(fam_dat) <-  c(as.character(gas_fam))


for (i in 1:length(fam_dat$age.ori)) {
  fam_dat[i, 2] <- min(gastropods$FAD_bin[gastropods$Family == gas_fam[i]])
...
gas_ord <- unique(gastropods$Order)
ord_dat <- data.frame(matrix(ncol =2 , nrow = length(gas_ord)))
colnames(ord_dat) <- c("age.ori", "bin.ori")
rownames(ord_dat) <-  c(as.character(gas_ord))


for (i in 1:length(ord_dat$age.ori)) {
  ord_dat[i, 2] <- min(gastropods$FAD_bin[gastropods$Order == gas_ord[i]])
...
#remov ori age and bin
three_trends_gastropods <- test4

# Set bins back to numeric
three_trends_gastropods %<>% transform(bins = as.numeric(as.character(bins)))

# Cleanup
rm(dumbo, fam_dat, gastropods, lin1, 
   ord_dat, ori_bin,  test2, test3, test4, age1, bin1, 
   i, gas_fam, gas_ord, namevector, temp1, temp_gastropods, gastropods_safe)
...
# Set bins back to numeric
three_trends_gastropods %<>% transform(bins = as.factor(as.character(bins)))

# Now bind the two
thirteen_trends_gastropods <- full_join(three_trends_gastropods, isotemp2)


# Set bins back to numeric
thirteen_trends_gastropods %<>% transform(bins = as.numeric(as.character(bins)))

# remove redundant colums
thirteen_trends_gastropods <- thirteen_trends_gastropods[-13142,]
